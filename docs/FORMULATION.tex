...
\mathcal{M}:
\left(
\F_n,\F_{n+1}^{(i)},\mathcal H_n,t_n,t_{n+1}
\right)
\longrightarrow
\left(
\Pstress_{n+1}^{(i)},
...
\widehat{\Pstress}
\left(
\F_{n+1};\F_n,\mathcal H_n,\Delta t
\right).
\label{eq:Phat-map}
\end{equation}
The hat does not denote a new stress measure. It denotes the algorithmically returned endpoint stress after the complete local integration from the committed state $(\F_n,\mathcal H_n)$ to the specified endpoint $\F_{n+1}$. With the committed arguments held fixed,
\begin{equation}
\boxed{
\Aalg_{n+1}
=
