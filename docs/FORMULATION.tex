...
\subsection{Superposed rigid rotations and reported strain measures}

Consider a proper orthogonal tensor $\bs Q$, with $\bs Q^\top\bs Q=\I$ and $\det\bs Q=1$, superposed on a deformation. The rotated motion has
\begin{equation}
\F^+=\bs Q\F,
\qquad
J^+=J.
\end{equation}
Objectivity of a hyperelastic response requires
\begin{equation}
W(\bs Q\F)=W(\F),
\qquad
\Pstress(\bs Q\F)=\bs Q\Pstress(\F),
\qquad
\sig(\bs Q\F)=\bs Q\sig(\F)\bs Q^\top.
\label{eq:superposed-rotation-stress}
\end{equation}
This endpoint covariance is the relevant large-rotation verification for the hyperelastic prototype. It tests the complete kinematic, constitutive, stress-transformation, and output path; it should not be described as a numerical integration test of a hypoelastic stress-rate equation.

The Green--Lagrange and Euler--Almansi strains are
\begin{equation}
\bs E=\frac12(\F^\top\F-\I),
\qquad
\bs e=\frac12(\I-\F^{-\top}\F^{-1}).
\end{equation}
They intentionally report different frames. Under the same superposed rotation,
\begin{equation}
\bs E^+=\bs E,
\qquad
\bs e^+=\bs Q\bs e\bs Q^\top.
\label{eq:superposed-rotation-strain}
\end{equation}
Thus a bar stretched along the reference $X_1$ direction and then rigidly rotated by $90^\circ$ in the $x_1$--$x_2$ plane retains its dominant $E_{11}$ component, whereas the dominant spatial stress and Euler--Almansi strain components move from $11$ to $22$. Reporting both strains makes this frame distinction explicit rather than labeling an unspecified tensor simply as ``strain.''

\subsection{Uniaxial-stress stretch and rotation benchmark}

A prescribed axial stretch with fixed lateral stretches is not a uniaxial-stress test.
For the compressible neo-Hookean energy used here, the Cauchy stress is
\begin{equation}
\sig=\frac{1}{J}\left[\mu(\F\F^\top-\bs I)+\kappa\ln J\,\bs I\right].
\end{equation}
Consider $\F_s=\operatorname{diag}(\lambda,a,a)$, where $a>0$ is the transverse stretch.
Vanishing lateral traction requires
\begin{equation}
\mu(a^2-1)+\kappa\ln(\lambda a^2)=0.
\label{eq:uniaxial-lateral-stretch}
\end{equation}
For positive $\mu$ and $\kappa$, its left-hand side is strictly increasing in $a$, ranges from
$-\infty$ to $+\infty$, and therefore has exactly one positive root. For tensile $\lambda>1$,
$\lambda^{-1/2}<a<1$: contraction is neither suppressed nor assumed exactly isochoric.
The resulting stress is
\begin{equation}
\sig_s=\sigma_a\,\bs e_1\otimes\bs e_1,
\qquad \sigma_a=\frac{\mu(\lambda^2-a^2)}{\lambda a^2}.
\end{equation}
After a superposed rotation $\F=\bs R_z(\theta)\F_s$,
\begin{equation}
\sig=\sigma_a
\begin{bmatrix}
...
\end{bmatrix}.
\end{equation}
Thus $\sigma_{11}=0$ and $\sigma_{22}=\sigma_a$ at $90^\circ$. This homogeneous solution can be driven
by prescribing its motion on the two end faces, including the compatible transverse contraction, while leaving
the lateral surfaces traction-free. Interior and lateral-face nodes not on the ends are equilibrium unknowns.
This avoids boundary layers due to rigid transverse clamps. Lateral strains remain nonzero despite zero lateral
stress; their values rotate for the spatial strain but not for Green--Lagrange strain.

\subsection{Six-component reporting convention}

For any reported symmetric tensor $\bs S$, use the unscaled representation
\begin{equation}
\vct s_{\mathrm{out}}=
\begin{bmatrix}S_{11}&S_{22}&S_{33}&S_{12}&S_{23}&S_{13}\end{bmatrix}^{\top}.
\label{eq:output-six-components}
\end{equation}
In particular, strain shear components are tensor entries, not engineering shear strains.
This reporting convention is separate from the engineering-shear vectors used in element assembly.
For symmetric tensors $\bs S$ and $\bs T$, the double contraction is
\begin{equation}
\bs S:\bs T=\vct s_{\mathrm{out}}^\top
\operatorname{diag}(1,1,1,2,2,2)\vct t_{\mathrm{out}}.
\end{equation}
Work and energy calculations must also use mutually conjugate stress and strain measures;
the shear weights alone do not make Cauchy stress conjugate to Green--Lagrange strain.
...
Equivalently, write the microscopic displacement as
\begin{equation}
\bs u(\bs X)=(\overline{\F}-\I)\bs X+\widetilde{\bs u}(\bs X),
\end{equation}
where $\widetilde{\bs u}$ is periodic on opposite faces. The opposite-face contributions of $\widetilde{\bs u}$ cancel in the boundary integral, giving
\begin{equation}
\boxed{
\frac{1}{|\Omega_0|}\int_{\Omega_0}\F\,\mathrm dV_0=\overline{\F}.
}
\label{eq:periodic-average-F}
\end{equation}
For a homogeneous body the fluctuation may vanish and the exact solution may be affine, so that case mainly checks pairing and affine reproduction. A heterogeneous core--matrix body generally develops a nonzero periodic fluctuation and distinct phase fields while still satisfying Eq.~\eqref{eq:periodic-average-F}; these three observations together form a more discriminating periodic-boundary-condition test.
...
For a periodic cell with no body force or separately applied nodal load, equilibrium gives
$\vct f^{\mathrm{int}}=\vct f^{\mathrm{con}}$.  Isoparametric completeness then provides a convenient
macroscopic nominal-stress diagnostic directly from the nodal constraint reactions:
\begin{equation}
\boxed{
\overline{\P}
=\frac{1}{|\Omega_0|}\int_{\Omega_0}\P\,\mathrm dV_0
=\frac{1}{|\Omega_0|}\sum_a \bs f^{\mathrm{con}}_a\otimes\bs X_a .
}
\label{eq:periodic-average-P-reaction}
\end{equation}
Indeed, $\sum_a \bs f^{\mathrm{int}}_a\otimes\bs X_a$ reduces to the volume integral of $\P$
because $\sum_a \bs X_a\otimes\Grad N_a=\I$.  Equation~\eqref{eq:periodic-average-P-reaction}
uses the complete assembled nodal reaction vector, including the anchor reaction; it does not assign physical
meaning to individual multiplier coordinates.  If body forces or independent nodal loads are present, their
force moments must be included and this reduced expression cannot be used unchanged.
...
When globalization is required, the same KKT solution is retained as the Newton search direction but the update is damped,
\begin{align}
\vct u^{(i+1)}&=\vct u^{(i)}+\alpha_i\Delta\vct u^{(i)},\\
\vct\lambda^{(i+1)}&=\vct\lambda^{(i)}+\alpha_i\Delta\vct\lambda^{(i)},
\qquad 0<\alpha_i\leq1.
\label{eq:kkt-damped-update}
\end{align}
This does not turn exact Newton into modified Newton: exact Newton still forms the tangent at every accepted trial iterate.  It changes only the length taken along the current Newton direction.

For the residual-based backtracking used in the necking examples, define the dimensionless merit at the base iterate as
\begin{equation}
M_i=\max\left(
\frac{\lVert\vct r_u^{(i)}\rVert_\infty}{\epsilon_{f,i}},
\frac{\lVert\vct r_c^{(i)}\rVert_\infty}{\epsilon_{c,i}}
\right),
\label{eq:kkt-merit}
\end{equation}
where $\epsilon_{f,i}$ and $\epsilon_{c,i}$ are the absolute-plus-relative convergence tolerances evaluated at that base iterate.  Starting with $\alpha_i=1$, reduce it geometrically until the candidate is kinematically admissible and
\begin{equation}
M(\alpha_i)\leq (1-c\alpha_i)M_i,
\qquad 0\leq c<1.
\label{eq:kkt-armijo}
\end{equation}
Every candidate constitutive update is evaluated from the same committed pair $(\F_n,\mathcal H_n)$, exactly as for an ordinary Newton trial.  Hence a rejected candidate cannot contaminate history.  A nonpositive trial Jacobian or another recoverable local failure rejects that search length; failure to find an admissible decreasing length is itself a recoverable increment failure and may invoke pseudo-time cutback.
...
=-(\vct C\vct u^{(i)}-\vct d).
\end{equation}
Therefore, after one accurately solved \emph{undamped} Newton correction the trial displacement satisfies the linear constraints to linear-solver tolerance. With Eq.~\eqref{eq:kkt-damped-update}, the constraint residual is instead multiplied by $1-\alpha_i$ and is removed over subsequent accepted corrections. Once the constraints are satisfied, subsequent corrections within the same increment satisfy $\vct C\Delta\vct u\simeq\vct0$ because $\vct d$ is fixed at the target pseudo-time.
...
\item For Newton iteration $i=0,1,\ldots,N_{\max}-1$:
  ...
  \item Apply the full correction, or, when globalization is enabled, select an admissible residual-reducing $\alpha_i$ by Eqs.~\eqref{eq:kkt-damped-update}--\eqref{eq:kkt-armijo}. Reevaluate every search candidate from the unchanged committed material state $\mathcal H_n$.
  \end{enumerate}
...
\appendix
\section{Example constitutive plug-in: finite-strain J2 plasticity}
\label{app:j2-plasticity}

This appendix defines one stateful material that can be supplied through the generic material-point interface.  It is deliberately outside the main formulation: none of the finite element, constraint, solution, or output equations depends on J2 plasticity.  Another material may replace this appendix's update provided it accepts the same kinematic inputs and returns the required stress, state, status, and endpoint tangent.

\subsection{Kinematics, elastic response, and history}

Use the multiplicative split
\begin{equation}
\F=\Fe\Fp,
\qquad
\det\Fp=1,
\qquad
\bs C_{\mathrm p}=\Fp^\top\Fp.
\end{equation}
The committed history is the inverse plastic metric $\bs C_{\mathrm p,n}^{-1}$ and accumulated equivalent plastic strain $\bar\varepsilon_{\mathrm p,n}$.  The cold state is
\begin{equation}
\bs C_{\mathrm p,0}^{-1}=\I,
\qquad
\bar\varepsilon_{\mathrm p,0}=0.
\end{equation}
No deformation gradient is stored as history.

For elastic Jacobian $J_{\mathrm e}=\det\Fe=J$ and unimodular elastic left metric
\begin{equation}
\bar{\bs b}_{\mathrm e}=J^{-2/3}\F\bs C_{\mathrm p}^{-1}\F^\top,
\qquad \det\bar{\bs b}_{\mathrm e}=1,
\end{equation}
the stored energy selected for this material is
\begin{equation}
W_{\mathrm e}
=\frac{K}{2}(J-1)^2
+\frac{\mu}{2}\left(\tr\bar{\bs b}_{\mathrm e}-3\right).
\label{eq:j2-energy}
\end{equation}
Thus the Kirchhoff stress is
\begin{equation}
\tauK=KJ(J-1)\I+\bs s,
\qquad
\bs s=\mu\dev\bar{\bs b}_{\mathrm e}.
\label{eq:j2-kirchhoff}
\end{equation}
This volumetric energy is specific to this plug-in and is not the coupled logarithmic-volume neo-Hookean reference material used elsewhere.

The isotropic Voce law with a linear tail is
\begin{align}
h(\bar\varepsilon_{\mathrm p})
&=\sigma_{y0}+H\bar\varepsilon_{\mathrm p}
+Q\left(1-\exp[-b\bar\varepsilon_{\mathrm p}]\right),\\
h'(\bar\varepsilon_{\mathrm p})
&=H+Qb\exp[-b\bar\varepsilon_{\mathrm p}],
\end{align}
and the yield function is
\begin{equation}
\Phi=\lVert\bs s\rVert-\sqrt{\frac23}\,h(\bar\varepsilon_{\mathrm p}).
\end{equation}
Here $\lVert\bs s\rVert=\sqrt{\bs s:\bs s}$ is the spatial Frobenius norm.

\subsection{Elastic predictor and radial return}

At every global Newton trial, hold the committed state fixed and form
\begin{align}
\bar{\bs b}^{\mathrm{tr}}
&=J_{n+1}^{-2/3}\F_{n+1}\bs C_{\mathrm p,n}^{-1}\F_{n+1}^{\top},\\
\bs s^{\mathrm{tr}}&=\mu\dev\bar{\bs b}^{\mathrm{tr}},
\qquad
q^{\mathrm{tr}}=\lVert\bs s^{\mathrm{tr}}\rVert,\\
\bar\mu&=\frac{\mu}{3}\tr\bar{\bs b}^{\mathrm{tr}}.
\end{align}
If $q^{\mathrm{tr}}-\sqrt{2/3}\,h(\bar\varepsilon_{\mathrm p,n})$ is not positive beyond the yield tolerance, accept the elastic predictor and leave history unchanged.  Otherwise solve the monotone scalar equation
\begin{equation}
r(\Delta\gamma)
=q^{\mathrm{tr}}-2\bar\mu\Delta\gamma
-\sqrt{\frac23}\,
h\left(\bar\varepsilon_{\mathrm p,n}+\sqrt{\frac23}\Delta\gamma\right)=0.
\label{eq:j2-return-scalar}
\end{equation}
Its derivative is
\begin{equation}
r'(\Delta\gamma)
=-2\bar\mu-\frac23 h'\left(\bar\varepsilon_{\mathrm p,n}+\sqrt{\frac23}\Delta\gamma\right)<0.
\end{equation}
The update is
\begin{align}
\beta&=1-\frac{2\bar\mu\Delta\gamma}{q^{\mathrm{tr}}},
&\bs s_{n+1}&=\beta\bs s^{\mathrm{tr}},\\
\bar\varepsilon_{\mathrm p,n+1}
&=\bar\varepsilon_{\mathrm p,n}+\sqrt{\frac23}\Delta\gamma.
\end{align}

The radial stress correction determines only the deviatoric part of the updated unimodular elastic metric.  Define $\bs q=\bs s_{n+1}/\mu$ and choose the unique scalar $c>-\lambda_{\min}(\bs q)$ satisfying
\begin{equation}
\det(\bs q+c\I)=1.
\label{eq:j2-unimodular-root}
\end{equation}
Then reconstruct
\begin{equation}
\bar{\bs b}_{\mathrm e,n+1}=\bs q+c\I,
\qquad
\bs C_{\mathrm p,n+1}^{-1}
=J_{n+1}^{2/3}\F_{n+1}^{-1}
\bar{\bs b}_{\mathrm e,n+1}\F_{n+1}^{-\top}.
\label{eq:j2-state-reconstruction}
\end{equation}
This documented spherical reconstruction leaves the radial stress correction unchanged while enforcing symmetry, positive definiteness, and $\det\bs C_{\mathrm p,n+1}^{-1}=1$ to numerical tolerance.  It avoids an ad hoc normalization of a nonsymmetric or indefinite state after the fact.

Finally,
\begin{equation}
\Pstress_{n+1}=\tauK_{n+1}\F_{n+1}^{-\top}.
\end{equation}

\subsection{Exact linearization of the discrete update}

The implemented tangent differentiates the preceding discrete return, not the continuum rate equations with frozen internal variables.  For an arbitrary direction $\bs H=\delta\F$, define
\begin{equation}
a=\tr(\F^{-1}\bs H),
\qquad \delta J=Ja.
\end{equation}
With committed $\bs C_{\mathrm p,n}^{-1}$ fixed,
\begin{align}
\delta\bar{\bs b}^{\mathrm{tr}}
={}&-\frac23a\bar{\bs b}^{\mathrm{tr}}
+J^{-2/3}\left(
\bs H\bs C_{\mathrm p,n}^{-1}\F^\top
+\F\bs C_{\mathrm p,n}^{-1}\bs H^\top\right),\\
\delta\bs s^{\mathrm{tr}}
={}&\mu\dev(\delta\bar{\bs b}^{\mathrm{tr}}),\\
\delta q^{\mathrm{tr}}
={}&\frac{\bs s^{\mathrm{tr}}:\delta\bs s^{\mathrm{tr}}}{q^{\mathrm{tr}}},
\qquad
\delta\bar\mu=\frac{\mu}{3}\tr(\delta\bar{\bs b}^{\mathrm{tr}}).
\end{align}
On a smooth plastic branch, implicit differentiation of Eq.~\eqref{eq:j2-return-scalar} gives
\begin{equation}
\delta\Delta\gamma
=\frac{\delta q^{\mathrm{tr}}-2\Delta\gamma\,\delta\bar\mu}
{2\bar\mu+\frac23h'(\bar\varepsilon_{\mathrm p,n+1})}.
\end{equation}
Consequently,
\begin{align}
\delta\beta
&=-2\left[
\frac{\Delta\gamma\,\delta\bar\mu+\bar\mu\,\delta\Delta\gamma}{q^{\mathrm{tr}}}
-\frac{\bar\mu\Delta\gamma\,\delta q^{\mathrm{tr}}}{(q^{\mathrm{tr}})^2}
\right],\\
\delta\bs s_{n+1}
&=\delta\beta\,\bs s^{\mathrm{tr}}+\beta\,\delta\bs s^{\mathrm{tr}}.
\end{align}
On the elastic branch, simply use $\delta\bs s_{n+1}=\delta\bs s^{\mathrm{tr}}$.  Both branches finish with
\begin{align}
\delta\tauK
&=K(2J-1)\delta J\,\I+\delta\bs s_{n+1},\\
\delta\Pstress
&=\delta\tauK\F^{-\top}
-\tauK\F^{-\top}\bs H^\top\F^{-\top}.
\label{eq:j2-tangent-action}
\end{align}
Equation~\eqref{eq:j2-tangent-action} is evaluated for the nine Cartesian basis directions to populate $A^{\mathrm{alg}}_{iIjJ}$.  This is an analytic directional construction of the exact algorithmic tangent; it is not finite differencing.  At the elastic--plastic switching surface the update is nonsmooth and a unique classical tangent is not required.  Centered finite differences away from that surface remain the independent verification oracle.

The trial state produced above is committed only after global convergence.  A cutback repeats the update from the unchanged $\bs C_{\mathrm p,n}^{-1}$ and $\bar\varepsilon_{\mathrm p,n}$.
...
The finite-strain J2 example in Appendix~\ref{app:j2-plasticity} follows the multiplicative hyperelastic-plastic framework and radial-return structure of J. C. Simo, ``Algorithms for static and dynamic multiplicative plasticity that preserve the classical return mapping schemes of the infinitesimal theory,'' \emph{Computer Methods in Applied Mechanics and Engineering}, 99, 61--112 (1992).  Its determinant-one spherical reconstruction is stated explicitly in Eqs.~\eqref{eq:j2-unimodular-root}--\eqref{eq:j2-state-reconstruction} as part of this solver's discrete material definition.
